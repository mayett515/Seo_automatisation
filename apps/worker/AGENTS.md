...
- Status honesty: `queued`, `completed`, `failed`, `dry_run`, `not_configured`, and `pending` describe the side effect that actually happened. Never return `queued` without a real enqueue, and never create durable job rows before the queue infrastructure that will execute them exists.
