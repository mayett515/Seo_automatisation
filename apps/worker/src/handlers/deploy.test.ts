...
void describe("executeDeploy", () => {
  ...
  void it("stops instead of replaying a verified healthy deployment that requires manual reconciliation", async () => {
    const data = deployJobData();
    const repository = createRepository(
      deployContext({
        existingDeployment: deploymentRow({
  ...
  void it("stops when start returns a replayable healthy row that requires manual reconciliation", async () => {
    const data = deployJobData();
    const repository = createRepository(deployContext(), {
      startDeploymentResult: deploymentRow({
        status: "live_healthy",
