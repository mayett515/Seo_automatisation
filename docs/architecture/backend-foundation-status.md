...
- Create rollback points from the current live deployment at the right lifecycle moment, with provider deploy id, artifact key, live URL, and enough evidence to restore.
...
- Separate verifier execution failure from observed live-page verification failure.
- Reject absolute/protocol-relative release target URLs before verification can fetch outside the deployment host.
