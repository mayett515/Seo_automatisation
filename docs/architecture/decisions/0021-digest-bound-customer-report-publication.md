...
Slice 5 implements the authenticated report workspace and completes the first fact-only vertical. Owner/admin/editor workspace reads expose bounded current candidates, latest generation status, stored canonical snapshots, and artifact summaries without storage locations; viewer access remains limited to the existing published list/detail/document reads. The React workspace uses TanStack Query, Form, Router, and Table for explicit generation, review, request-changes, render retry, publication/correction, and immutable history states. Candidate and published HTML are displayed only through sandboxed document routes that re-verify immutable artifact bytes. Authenticated candidate/published detail reads mint five-minute capabilities bound to the exact project, report, artifact, snapshot digest, and artifact digest. The document routes rely on those HttpOnly, Secure, `SameSite=None`, Partitioned cookies rather than assuming application-session cookies survive an opaque-origin or cross-site iframe; a real-network Chromium test proves the chain. V1 Next Actions remain typed navigation to Opportunity, Page Studio review, or Release review; the UI cannot submit arbitrary URLs or execute downstream commands.

Slice 6 implements optional bounded AI headings and transitions without changing report truth ownership. The deterministic packet and fact projection are built first. The server then assigns exact fact-light narrative slots; the provider may return only `slotKey` plus text, and deterministic QA restores server-owned kind/supporting-claim metadata after rejecting missing slots, markup, URLs, factual tokens, guarantees, causal claims, and duplicate text. One `report_narrative` agent run reuses the report-generation run id and is scoped by non-null `subjectId = reportId`. A bounded-AI snapshot must match that exact succeeded attributed output at the database boundary. Provider, storage, or QA failure completes the same generation as an honest fact-only report. The existing report queue and recovery lane own the work; bounded exhaustion terminalizes the parent generation and active narrative audit together. The HTML renderer places attributed fragments only into their deterministic server-owned slots. Because that placement and stylesheet change immutable bytes, migration `0044_customer-report-renderer-v2` advances both identities to v2; its artifact trigger permits historical v1 rows to finish or expire but requires v2 for every new artifact. Command offers, PDF, RAG, public links, and direct report prose editing remain deferred.

The first command-offer prerequisite hardens `request_page_proposal` without introducing report action rows. A DB-managed opportunity row version is forced to zero on insert and advances on every update; the existing Page Proposal command now requires the displayed opportunity status/revision, locks and re-reads the project-owned target, and creates the durable run only when the expected target still matches and remains eligible. Queue-unavailable dry-run audit validates the same target first, while worker success persistence shares the structural `opportunity -> agent_run` lock order. Forced PostgreSQL lifecycle-update/request and worker-persistence lock interleavings prove the stale loser creates neither run nor queue work and the worker owns the target before changing run truth. `page:propose`, the existing active-run guard, Page Studio review, and every downstream approval/release boundary remain unchanged. `prepare_release_plan` target hardening and report offer/receipt/dispatch persistence remain later slices.
