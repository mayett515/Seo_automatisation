...
Page Proposal admission is expected-state/revision bound. PostgreSQL owns `opportunities.row_version`, forces revision zero on insert, and advances it on every update. The Explorer sends the displayed opportunity status and row version with the proposal request; the API locks and re-reads the project-owned opportunity, applies the pure target-admission decision, and creates the durable `page_brief_draft` run in the same transaction only when both values still match. Queue-unavailable dry-run audit performs that admission check before writing durable job truth. A lifecycle update that wins first makes the request stale, while duplicate requests at the same revision still coalesce through the existing active-run guard. The worker success transaction follows the same `opportunity -> agent_run` lock order before proposal/version persistence. Rejected opportunities and opportunities already at `brief_created` remain server-side ineligible. This is the first target CAS prerequisite for ADR 0021 command offers; it does not create report offer, consent, or dispatch truth.
...
Media uses project-scoped asset references, not URLs. ADR 0020 fixes the boundary: Postgres owns asset identity/lifecycle/actor evidence, private object storage owns uploaded and normalized bytes, and PageJson may carry only a strict placement reference with an opaque asset id, content-vs-decorative alt semantics, and an optional normalized focal point. Uploads remain private quarantine objects until a deterministic, recoverable worker validates decoded content, strips metadata, and writes immutable derivatives. Preview and deploy resolve the same project-scoped manifest and emit the same immutable root-relative asset paths. `ImageText` is the first registry-owned media section. PageJson media references are renderer selection truth, while `page_version_media_assets` is an exact transactionally maintained retention/FK projection. New edits may select only ready project assets; direct descendants may retain archived references from their base version. Approval, release planning, preflight, preview, and deploy all re-resolve exact project-scoped manifests and fail closed on reference/projection/byte drift. Upload completion never edits PageJson; only explicit application through the existing versioned command creates N+1.
...
Release-plan creation is the first consumer of approved page versions. PostgreSQL owns `page_versions.row_version`, forces revision zero on insert, and advances it on every lifecycle update. The request carries each displayed page-version status/revision; the API locks project-owned targets in stable id order, re-reads them, and rejects stale state before eligibility or persistence. It may create draft release plans and pending release items only from `page_versions.status = "approved"` rows with `approved_at` evidence. It must reject preview, changes-requested, release-candidate, released, superseded, missing, cross-project, approval-evidence-missing, or stale page versions. Release planning does not approve deploy, enqueue deploy, mutate providers, or mark a release live.
...
23. Harden the Page Proposal target for later consequential report actions: expose a DB-managed opportunity row version, require the displayed status/revision at request admission, lock and re-read before durable run creation, and reject stale, rejected, or already-completed targets. Done in the first ADR 0021 command-offer prerequisite slice; report offer/receipt/dispatch persistence remains separate.
24. Harden release-plan preparation for later consequential report actions: expose a DB-managed page-version revision, require every selected status/revision, lock targets in stable order, and reject a concurrent lifecycle winner before draft-plan persistence. Done in the second ADR 0021 command-offer prerequisite slice; report offer/receipt/dispatch persistence remains separate.
