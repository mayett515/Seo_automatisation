...
17. Add controlled section replacement through the existing command/version boundary: registry-derived legal targets and prop controls, local staging, explicit confirmation, server-derived structure, stable slot identity/order, and the existing contract/registry/composition/render gates. Done in the controlled section-replacement slice; media and AI copy actions remain follow-ups.
