...
Release-plan creation is expected-state bound before the plan exists. The request freezes each displayed page-version status and DB-managed row version. Contract-valid UUIDs are canonicalized to lowercase before JavaScript ordering so request-side lock order matches PostgreSQL `uuid` order. The transaction locks project-owned targets in stable page-version-id order, re-reads them, rejects stale state before approval eligibility, and only then inserts the new draft plan and items. Page versions are the terminal lock class in the established `rollback_points -> deployments -> release_plans -> page_versions` order; creation does not acquire an existing release-plan lock after taking target locks.

Release-plan approval and every shared release-candidate demotion path preserve that terminal class by locking all affected page versions in ascending id order after the owning plan transition. The demotion helper materializes and locks its sorted target set before updating status, so cancellation, re-preflight, deploy failure, verification failure, and rollback cannot acquire the same page-version set in plan-dependent order.
