...
Preview and deploy use the same PageJson, resolved manifest, and renderer core. Preview transport uses an authenticated preview-document URL rather than `iframe srcDoc`, so root-relative `/assets/...` requests resolve through a capability-authorized API asset handler. The deployed artifact uses the identical asset paths and includes the same derivative bytes at those paths. Editor preview may add its deliberate `noindex` policy; deploy-preview mode remains byte-identical to the deploy HTML for the same PageJson and target URL.

The preview iframe keeps `sandbox=""`; it must not gain `allow-same-origin` or `allow-scripts`. Normal application-session cookies are not the authorization boundary for the opaque-origin document or its media subresources. The ordinary session/project-authorized preview metadata endpoint resolves the exact immutable manifest and issues a page-version-scoped document capability cookie. The capability-only document endpoint verifies that cookie and the current manifest hash before returning renderer HTML, then issues a second page-version-scoped capability cookie restricted to `/assets`. The asset handler accepts only a valid asset capability whose current manifest contains the requested immutable path and verifies the downloaded bytes against that manifest before responding. Both capabilities bind project id, page-version id, exact manifest hash, and expiration. Capability cookies use `HttpOnly`, `Secure`, `SameSite=None`, `Partitioned`, a maximum five-minute lifetime, and `Cache-Control: private, no-store` in every environment. `SameSite=Lax` is not permitted even behind the local same-origin proxy because requests originating in `sandbox=""` have an opaque site-for-cookies. Chromium accepts `Secure` cookies on trustworthy `http://localhost`; the browser suite proves the document-to-assets chain over a real network request. Partitioning keeps embedded capabilities bound to the operator app's top-level site rather than becoming reusable third-party cookies. Local development uses the Vite same-origin `/api` and `/assets` proxies while preserving the same capability checks. No token enters the HTML or asset path.

`StaticSiteFile` is binary-safe through an explicit discriminated content encoding (`utf8` or `base64`) rather than an optional second body field. This keeps each approved static artifact self-contained and provider-neutral. Artifact construction rejects duplicate or unsafe paths and enforces a 50 MiB decoded-byte budget before persistence/provider handoff. The hosting adapter decodes each file exactly once, calculates Netlify's SHA1 over the decoded byte buffer, and uploads that same byte buffer. It never hashes or uploads the base64 transport string. The deploy worker resolves immutable projected media bytes, verifies byte count and SHA-256, and builds the complete artifact; the hosting adapter only parses, decodes, hashes, and uploads artifact bytes and must not import media, registry, or database logic.
...
- Preview/deploy parity uses a capability-authorized preview-document route and a binary-safe static artifact contract.
