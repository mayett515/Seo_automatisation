...
Migration 0051 is a pre-activation hardening migration: it intentionally aborts if a succeeded workflow checkpoint already exists because historical rows do not retain the exact canonical bytes needed for a truthful backfill. Deploy migrations 0047-0051 before enabling Opportunity Research workers. If an interrupted rollout allows 0047-0050 work to succeed first, stop those workers and perform an explicit operator-reviewed ledger reset or canonical-byte backfill before retrying 0051; do not synthesize canonical text from `jsonb::text`.
