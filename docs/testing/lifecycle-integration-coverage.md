...
2. `canonicalize@3.0.0` produces identical RFC 8785 text after deterministic semantic-array ordering, including 100 property-based generated permutations of top-level and nested semantic arrays.
...
The DB-backed aggregate suite proves:

1. Two first-generation requests racing an absent issue converge on one stable issue and one active generation run.
2. Snapshot admission recomputes exact canonical digests, persists one report plus normalized claims/evidence/links and a `report_generated` event transactionally, rejects unapproved proof tiers and cross-issue generation references at the database boundary, and keeps publication absent.
3. Submitting for review requires exact canonical claim/evidence/link sets rather than count equality, freezes snapshot/provenance writes, and rejects forged lifecycle events; request-changes returns the exact digest to draft with bounded persisted human evidence.
4. Draft regeneration requires the admitted issue/report row version and digest, replaces normalized provenance atomically, and keeps one report version/open candidate.
...
The worker integration cases prove that all seven allowed evidence kinds become one project-scoped canonical bounded packet, only the latest terminal verification contributes health, internal/GSC/raw check text stays out, production-shaped rollback points resolve the rolled-back plan target while incomplete preflight rows are skipped, rolled-back deployments do not claim provider handoff, duplicate delivery coalesces to one draft/event, and deterministic fact-only claims/actions persist without an AI adapter. Bounded-narrative cases prove exact report-scoped agent attribution, deterministic exact-slot QA, fact-only degradation after provider failure or QA rejection, database rejection of provenance/output drift, and parent-recovery terminalization of active narrative audit. Unit coverage proves the model output remains narrower than server-owned slot metadata, rejects adjacent-character Markdown, and the renderer rejects fragments outside deterministic layout slots. The stale-work suite proves same-run re-enqueue plus visible bounded exhaustion and completed-transport inconsistency. Migration `0042_report-canonical-collation-restored` owns the latest review-trigger definition and pins evidence/link aggregation to locale-independent code-unit ordering; the report integration suite inspects the installed function to prove both pins survived migration ordering. Migration `0043_customer-report-narrative-provenance` binds every bounded snapshot to one exact succeeded report-scoped agent output while keeping fact-only reports free of narrative provenance. Migration `0044_customer-report-renderer-v2` advances changed renderer/stylesheet byte identities, permits historical v1 artifact transitions, and rejects new artifacts that do not use v2.
...
Slice 5 adds strict bounded workspace/candidate response coverage, project-scoped candidate and artifact reads without storage locations, and pure UI-state tests for generation windows, active-run polling, and typed navigation targets. Chromium at 390px proves the explicit draft -> review -> staged immutable document -> publication -> published document flow, asserts the two digest-bound commands, and rejects horizontal viewport overflow. A separate real-network Chromium case proves an authenticated detail response's `Secure; SameSite=None; Partitioned` capability reaches the cross-site `sandbox=""` report document and validates the exact project/report/artifact/snapshot/artifact-digest binding. Published history remains readable when the operator workspace permission is unavailable.
