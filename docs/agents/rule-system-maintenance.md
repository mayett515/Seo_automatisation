...
- Cursor-only rules, subagents, and hooks live in `.cursor/`. Codex-only counterparts live in `.codex/` when installed. The Claude-native layer lives in `CLAUDE.md` plus `.claude/` (path-scoped rules, pack skills, the diff-reviewer subagent), copied verbatim from the pack master, plus four hooks wired in `.claude/settings.json`. The rule and skill counts are not written here; `ls .claude/rules | wc -l` and `ls .claude/skills | wc -l` answer, and the guards enforce the floor. Three of those hooks come from the pack (`protect-paths`, `guard-bash`, `post-edit-lint`); `post-edit-checks` is project-owned, because the checks it runs are this repository's own.
...
- Skill ownership: `.agents/skills/` is canonical for the project skills listed below, and `.claude/skills/` carries the pack master's copies for Claude-native discovery. Every name in `.claude/skills/` also exists in `.agents/skills/`, so "a name in both trees" describes all of them rather than a special case; what separates them is whether the two copies agree. Most are byte-identical and stay that way. A handful are deliberately different, because the project adapted the pack version: those are declared in `tools/check-architecture-regression-guards.ts` (`SKILL_MIRROR_DIVERGENCE`) with a reason each, and an undeclared difference fails the build. Count them with `ls .claude/skills | wc -l` rather than trusting a number written here; a frozen count outlives its truth. Sync deliberately through pack updates, never by ad-hoc edits.
- `source-of-truth-audit` diverges from the two external translation packs on purpose. Its `.agents/` copy here carries the rename-acceptance step added to the Claude pack master (`C:\claude\claude-workflows\typescript`), while `C:\codex\codex-workflows\typescript` and `C:\cursor\cursor-workflows\typescript` were deliberately left untouched. The pack master's `check-parity.mjs` binds those two external copies to this repository's `.agents/` copy byte-for-byte, so it reports one `DRIFT skill source-of-truth-audit`. That is a recorded, accepted divergence, not a broken build: the checker is run by hand from the pack master and is not part of this repository's CI. Re-syncing the external packs would close it; nobody has asked for that.
