...
export type DeepSeekReasoningAdapterOptions = {
  apiKey: string;
  model: string;
  baseUrl?: string;
...
type DeepSeekFailureCode = Extract<AiReasoningRunResult, { ok: false }>["failureCode"];

const defaultDeepSeekBaseUrl = "https://api.deepseek.com";

export class DeepSeekReasoningAdapter implements AiReasoningPort {
  private readonly endpoint: string;
  private readonly fetchImpl: typeof fetch;

  constructor(private readonly options: DeepSeekReasoningAdapterOptions) {
    this.endpoint = `${(options.baseUrl ?? defaultDeepSeekBaseUrl).replace(/\/$/u, "")}/chat/completions`;
...
function failureCodeFromHttpStatus(status: number): DeepSeekFailureCode {
...
function isTimeoutProviderError(error: unknown): boolean {
  return isProviderRequestError(error) && error.provider === "deepseek" && error.reasonCode === "timeout";
}
