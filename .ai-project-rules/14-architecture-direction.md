...
- Before writing an ADR or shaping a new vertical slice, run the inspiration-pass domain scan (the skill reads its archived domain map on demand) for cross-cutting concerns and quality attributes the user may not have named.
</positive-directives>
...
THEN run the inspiration-pass domain scan before finalizing the design.
</conditional-logic>
